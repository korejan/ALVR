    ...
    unsafe {
        ...
        match build_no {
            50801630051100340 | 50801630046600340 => {
                // quest v77.0.0.x
                log::warn!(
                    "alxr-client: force disabling XR_KHR_visibility_mask, quest crash bug workaround for build-no.: {build_no}"
                );
        ...
        log::info!(
            "alxr-client: Overriding OpeXR recommend eye resolution ({}x{}) with preferred resolution ({eye_w}x{eye_h})",
            sys_properties.recommendedEyeWidth,
            sys_properties.recommendedEyeHeight
        );
